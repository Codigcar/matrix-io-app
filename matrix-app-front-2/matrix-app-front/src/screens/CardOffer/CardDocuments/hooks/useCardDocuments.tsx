...
const useCardDocuments = (props: NavigationPropsType) => {
  const { navigation } = props;
  const [loading, setLoading] = useState<boolean>(false);

  const onDownloadDocument = async (title: string, url: string, needEncryptionFromAppDome: boolean = false ) => {
    ...
    try {
      const localFileUrl = await downloadFromUrl(url, title, needEncryptionFromAppDome);
      const shareOptions = {
        title: title,
        failOnCancel: false,
        saveToFiles: false,
