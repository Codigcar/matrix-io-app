...
import { signInEmailSelector } from '../selectors/paymentSelector';
import GetPaymentMethod from '../services/getPaymentMethodStatus';
import { CardPaymentProps, getPaymentStatusResponse } from '../types/types';
import useErrorsValidation from './useErrorsValidation';
...
export const useCardPayment = (props: CardPaymentProps & NavigationPropsType) => {
  ...
  const pollStatus = async (methodId: string): Promise<getPaymentStatusResponse> => {
    const cardStatus = await GetPaymentMethod.getPaymentStatus(methodId);
    if (
  ...
  const onSubmit = async () => {
    try {
      const cardInitialPayment = await GetPaymentMethod.cardPayment(
        values.route.params.sendPayload,
      );
