...
const CashbackCardSoon: FC<IInfoModalPropsType> = ({ isVisible, onClose }) => {
  ...
  const minRedemptionPoints = useSelector((state) => state.redemption.rules.minRedemptionPoints);
  ...
  const accumulatedCashback = useSelector((state) => state.redemption.accumulatedCashback);
  ...
  const goToPayMyCard = () => {
    navigation.navigate(navigationScreenNames.redemption.stack);
  };
