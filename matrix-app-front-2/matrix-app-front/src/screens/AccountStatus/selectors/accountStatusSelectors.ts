...
export const accountBalanceSelector = (state: AccountStatusProps) => {
  if (!Array.isArray(state?.account?.balance?.getReqBalanceSuccess)) return [];
  return state?.account?.balance?.getReqBalanceSuccess;
};

export const getBalanceSelector = (state: any) => state?.account?.balance?.getReqBalanceSuccess;

export const accountBalanceLoading = (state: AccountStatusProps) => state
  ?.account?.balance?.isLoading;

export const accountBalanceError = (state: AccountStatusProps) => state
  ?.account?.balance?.getReqBalanceError;

export const accountOrdersSelector = (state: AccountStatusProps) => state
  ?.account?.orders?.getReqOrdersSuccess;

export const accountOrdersErrorSelector = (state: RootState) =>
  state?.account?.orders?.getReqOrdersError;

export const accountOrdersLoading = (state: AccountStatusProps) => state
  ?.account?.orders?.isLoading;
...
export const accountOrderSelector = (
  ...
  const searchPendingPaymentOrders = paymentOrders
    .filter(
      ...
      }: PaymentOrder) => type === paymentOrdersTypes.ORDER_CLOSE
  ...
  const hasPendingPaymentOrders = searchPendingPaymentOrders.length > 0
    && searchPendingPaymentOrders[0].minimum.amount > 0;
