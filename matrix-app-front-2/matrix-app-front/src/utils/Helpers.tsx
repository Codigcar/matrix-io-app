...
const Helpers = {
  ...
  formatCurrency: (
    value: number,
    { removeDecimalsWhenRounded, showPlusSign }: formatCurrencyParams = {},
  ): string => {
    ...
    const formatOptions =
      removeDecimalsWhenRounded && removeDecimals
        ? {
    ...
    return `${sign}S/${Math.abs(value).toLocaleString('es-PE', formatOptions)}`;
  },
  formatStringCamel: (name: string): string => {
    const nameList = name.split(' ');
    return `${nameList.reduce(
      (prev, curr) =>
        `${prev + curr.split(' ')[0].charAt(0).toUpperCase() + curr.slice(1).toLowerCase()} `,
      '',
    )}`;
  },
  formatMoney: (
  ...
  capitalizeWord: (text: string) =>
    text
      ...
      .join(' '),
  separateNameInTwoLines: (text: string) =>
    text
      .toLocaleLowerCase()
      .trim()
      .split(/\s+/)
