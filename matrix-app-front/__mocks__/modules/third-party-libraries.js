...
jest.mock('react-native', () => {
  ...
  RN.NativeModules.RNi2cModule = {
    startTask: jest.fn(),
    finishTask: jest.fn(),
    cleanInterval: jest.fn(),
    finishTaskTimeOut: jest.fn(),
  };

  RN.NativeEventEmitter = { addListener: jest.fn() };
  return RN;
});
...
jest.mock('src/components/I2cSdk/I2CModule', () => ({
  ...
  changePinPhysicalCard: jest.fn(),
  finishTask: jest.fn(),
  cleanIOsInterval: jest.fn(),
  cleanAndroidInterval: jest.fn(),
  finishTaskTimeOut: jest.fn(),
}));
jest.mock('react-native-modalize');
...
jest.mock('react-native-snap-carousel', () => {
  const RealSnapCarousel = jest.requireActual('react-native-snap-carousel');
  const Pagination = jest.fn(() => null);
