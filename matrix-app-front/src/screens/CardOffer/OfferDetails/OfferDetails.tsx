...
const OfferDetails: React.FC<NavigationPropsType> = (props) => {
  ...
  const {
    offer, loading, setLoading, submitCardContract, name, summarySheet, startPolling,
  } = useCardOffer(props);
  ...
  const formatName = (names: String) => {
    try {
      if (names) {
        return names.split(' ')[0][0].toUpperCase() + names.split(' ')[0].slice(1).toLowerCase();
      }
      return names;
    } catch {
      return '';
  ...
  const alertModal = () => {
    switch (loading.type) {
      case 'warning':
        return (
          <Alert type={loading.type} onClick={onPressContinue} testID={testID.warningModalId} />
        );
      case 'error':
        return (
          <Alert
            type={loading.type}
            onClick={() => setLoading({ visible: false })}
            testID={testID.errorModalId}
          />
        );
      default:
        ...
        <SafeAreaView>
          <Box flex={1} mx="spacing-m" pt="spacing-xxxs" justifyContent="center">
            ...
            >
              ...
              {string.cardOfferTitle}
              <Text variant="Heading18SemiBold">{string.appName}</Text>
              {string.cardOfferTitleLast}
            </Text>
            ...
            >
              {string.cardOfferAmountText}
            </Text>

            <BenefitList offer={offer} />
            <Box>
              <CheckBox
                testID={testID.btnTermsId}
                onPress={() => {
                ...
                analytics={{
                  valor: string.cardOfferContractFirstText,
                }}
              >
                <Text
                  textAlign="left"
                  ...
                  analytics={{
                    valor: string.cardOfferContractLinkText,
                  }}
                >
                  {string.cardOfferContractFirstText}
                  <Text
                  ...
                  >
                    {string.cardOfferContractLinkText}
                  </Text>
                </Text>
              </CheckBox>
            </Box>
            <Button
              testID={testID.btnContinueGetOffer}
              variant={termsConditionsContract && !isButtonLoading ? 'primary' : 'disabled'}
              my="spacing-m"
              onPress={onPressContinue}
              loading={isButtonLoading}
              label={string.cardOfferButtonLabel}
              disabled={!termsConditionsContract || isButtonLoading}
            />
          </Box>
        </SafeAreaView>
        <ProcessingModal isVisible={loading.visible} testID={testID.loadingModal}>
          {alertModal()}
        </ProcessingModal>
      </Container>
    </ThemeProvider>
