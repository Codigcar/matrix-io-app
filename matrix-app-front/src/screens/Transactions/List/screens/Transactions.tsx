...
import { TransactionGroup, TransactionProps } from 'src/api/types/TransactionTypes';
...
import { useMovementDate } from 'src/shared/hooks';
...
export const TransactionsScreen = (props: NavigationPropsType) => {
  ...
  const { listMovements } = useMovementDate();
  ...
  const renderFooter = () => {
    ...
    if (reachedEnd) {
      return <Box alignItems="center" marginVertical="spacing-s" />;
    }
  ...
  const sections = transactionsFiltered.map((transacton) => ({
