...
export const useSigInPresenter = (propsNavigation: NavigationPropsType, identitySaved?: string) => {
  ...
  const {
    ...
    optionalUpdateModal: showOptionalUpdateModal,
  ...
  const checkOnboardingEvent = (isUserAvailable: boolean) => {
    ...
    if (response) {
      ...
      if (response.user) {
        ...
        const multiCurrencyStatus = null;
        ...
        dispatch(setMultiCurrencyStatus(multiCurrencyStatus));
      }
  ...
  const verifyAuthScreen = (): boolean => Boolean(
    (navigationRef?.current?.getState()?.routes || [])
      .find((row: { name: string }) => row.name === AuthRoutesEnum.AUTH_STACK) || false,
  );
  ...
  const checkIfNeedsForceUpdate = () => {
    ...
    if ((resultForceUpdate || '')?.length > 0) {
      ...
      if (verifyAuthScreen() && remoteVersion) {
        setMinRemoteVersion(remoteVersion);

        const isForceUpdate = semver.gt(remoteVersion.minVersion, currentVersion);
        const isOptionalUpdate = semver.gt(remoteVersion.suggestedVersion, currentVersion);

        if (!showForceUpdateModal && isForceUpdate) {
          updateForceUpdateModal(true);
        } else if (!showOptionalUpdateModal && isOptionalUpdate) {
          updateOptionalUpdateModal(true);
        }
      }
    }
  };
  ...
  const onCloseOptionalUpdateModal = () => updateOptionalUpdateModal(false);
  ...
  const goToMaintenanceScreen = () => {
    if (isMaintenance && verifyAuthScreen()) {
