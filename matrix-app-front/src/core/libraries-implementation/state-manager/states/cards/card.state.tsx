...
import { CARD_IS_OPEN, CARD_IS_INACTIVE, CARD_IS_STOLEN } from 'src/utils/constants';
import type { IMultiCurrencyStatus } from 'src/api/types/userTypes';
...
export const cardsSlice = createSlice({
  ...
  reducers: {
    setCards: (state, action) => {
      ...
      state.statusPhysicalCard = action.payload.cards[0].status === CARD_IS_INACTIVE
    ...
    setMultiCurrencyStatus: (state, action) => {
      state.multiCurrencyStatus = null;
      state.isMultiCurrency = Boolean(action.payload);
    },
  },
